            ...
            <div class="w-full  lg:w-3/4  flex flex-col gap-y-8 ">
                ...
                <form action="{{ route('cart.update-qty') }}" method="POST">
                    ...
                    >
                        ...
                        @foreach($userCartItems as $cartItems)
                            <!-- PRODUCT ITEM -->
                            <div class="w-full flex justify-between relative border-b-2 border-gray-200 dark:border-white/20 ">
                                <div class="flex flex-col sm:flex-row items-center gap-6">
                                    ...
                                    <div class="flex w-fit flex-col">
                                        ...
                                        <button
                                            ...
                                            <input
                                                type="number"
                                                name="cart_items[{{ $cartItems['product_id'] }}][qty]"
                                                data-cart-product-id="{{ $cartItems['product_id'] }}"
                                                id="qtyInput"
                                                min="1"
                                                max="50"
                                                value="{{ $cartItems['qty'] }}"
                                            ...
                                            />
                                            <input type="hidden" name="cart_items[{{ $cartItems['product_id'] }}][product_id]" value="{{ $cartItems['product_id'] }}">
                                            <svg
                                    ...
                                    <div class="flex flex-col gap-y-4">
                                        <h2 class="font-DanaMedium line-clamp-1">
                                            ...
                                            |
                                            {{ $cartItems['product']->name_en }}
                                        </h2>
                                        <div class="flex items-center gap-x-1 text-gray-700 dark:text-gray-300 font-DanaMedium mt-4">
                                            @if($cartItems['product']->discount)
                                                <div class="product-card_price">
                                                    <del>
                                                        <div
                                                            id="price-{{ $cartItems['product_id'] }}"
                                                            data-price="{{ $cartItems['product']->price * $cartItems['qty'] }}"
                                                            data-qty="{{ $cartItems['qty'] }}"
                                                            data-has-discount="true"
                                                            data-base-discount="{{ $cartItems['product']->discount }}"
                                                        >
                                                            {{ number_format($cartItems['product']->price * $cartItems['qty']) }}
                                                        </div>
                                                        <h6>تومان</h6>
                                                    </del>
                                                    <p
                                                        id="final-price-{{ $cartItems['product_id'] }}"
                                                    >
                                                        {{ number_format(($cartItems['product']->price - $cartItems['product']->discount) * $cartItems['qty']) }}
                                                    </p>
                                                    <span>تومان</span>
                                                </div>
                                            @else
                                ...
                                <div class="hidden sm:flex items-end justify-between flex-col">
                                    <a href="{{ route('cart.remove-item', $cartItems['product_id']) }}">
